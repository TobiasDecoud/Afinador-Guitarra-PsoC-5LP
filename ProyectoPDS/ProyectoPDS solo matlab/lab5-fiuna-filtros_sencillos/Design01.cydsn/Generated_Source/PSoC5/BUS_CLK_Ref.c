...
/*******************************************************************************
* Function Name: BUS_CLK_Ref_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BUS_CLK_Ref_Start(void) 
{
    /* Set the bit to enable the clock. */
    BUS_CLK_Ref_CLKEN |= BUS_CLK_Ref_CLKEN_MASK;
	BUS_CLK_Ref_CLKSTBY |= BUS_CLK_Ref_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: BUS_CLK_Ref_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BUS_CLK_Ref_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    BUS_CLK_Ref_CLKEN &= (uint8)(~BUS_CLK_Ref_CLKEN_MASK);
	BUS_CLK_Ref_CLKSTBY &= (uint8)(~BUS_CLK_Ref_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: BUS_CLK_Ref_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BUS_CLK_Ref_StopBlock(void) 
{
    if ((BUS_CLK_Ref_CLKEN & BUS_CLK_Ref_CLKEN_MASK) != 0u)
    {
...
#if defined(BUS_CLK_Ref__CFG3)
        CLK_DIST_AMASK = BUS_CLK_Ref_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = BUS_CLK_Ref_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* BUS_CLK_Ref__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(BUS_CLK_Ref_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(BUS_CLK_Ref_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: BUS_CLK_Ref_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BUS_CLK_Ref_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        BUS_CLK_Ref_CLKSTBY &= (uint8)(~BUS_CLK_Ref_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: BUS_CLK_Ref_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BUS_CLK_Ref_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = BUS_CLK_Ref_GetSourceRegister();
    uint16 oldDivider = BUS_CLK_Ref_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = BUS_CLK_Ref_CLKEN & BUS_CLK_Ref_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((BUS_CLK_Ref_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: BUS_CLK_Ref_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 BUS_CLK_Ref_GetDividerRegister(void) 
{
    return CY_GET_REG16(BUS_CLK_Ref_DIV_PTR);
}


/*******************************************************************************
* Function Name: BUS_CLK_Ref_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BUS_CLK_Ref_SetModeRegister(uint8 modeBitMask) 
{
    BUS_CLK_Ref_MOD_SRC |= modeBitMask & (uint8)BUS_CLK_Ref_MODE_MASK;
}


/*******************************************************************************
* Function Name: BUS_CLK_Ref_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BUS_CLK_Ref_ClearModeRegister(uint8 modeBitMask) 
{
    BUS_CLK_Ref_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(BUS_CLK_Ref_MODE_MASK));
}


/*******************************************************************************
* Function Name: BUS_CLK_Ref_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 BUS_CLK_Ref_GetModeRegister(void) 
{
    return BUS_CLK_Ref_MOD_SRC & (uint8)(BUS_CLK_Ref_MODE_MASK);
}


/*******************************************************************************
* Function Name: BUS_CLK_Ref_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BUS_CLK_Ref_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = BUS_CLK_Ref_GetDividerRegister();
    uint8 oldSrc = BUS_CLK_Ref_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        BUS_CLK_Ref_MOD_SRC |= CYCLK_SSS;
        BUS_CLK_Ref_MOD_SRC =
            (BUS_CLK_Ref_MOD_SRC & (uint8)(~BUS_CLK_Ref_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: BUS_CLK_Ref_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 BUS_CLK_Ref_GetSourceRegister(void) 
{
    return BUS_CLK_Ref_MOD_SRC & BUS_CLK_Ref_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: BUS_CLK_Ref_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BUS_CLK_Ref_SetPhaseRegister(uint8 clkPhase) 
{
    BUS_CLK_Ref_PHASE = clkPhase & BUS_CLK_Ref_PHASE_MASK;
}


/*******************************************************************************
* Function Name: BUS_CLK_Ref_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 BUS_CLK_Ref_GetPhaseRegister(void) 
{
    return BUS_CLK_Ref_PHASE & BUS_CLK_Ref_PHASE_MASK;
}
