...
void isr_dma_done_Stop(void)
{
    ...
    isr_dma_done_SetVector(&IntDefaultHandler);
}
...
{
    #ifdef isr_dma_done_INTERRUPT_INTERRUPT_CALLBACK
        isr_dma_done_Interrupt_InterruptCallback();
    #endif /* isr_dma_done_INTERRUPT_INTERRUPT_CALLBACK */ 
...
void isr_dma_done_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)isr_dma_done__INTC_NUMBER] = address;
}
...
cyisraddress isr_dma_done_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)isr_dma_done__INTC_NUMBER];
}
...
*
*   Note calling isr_dma_done_Start or isr_dma_done_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after isr_dma_done_Start or isr_dma_done_StartEx has been called. 
...
uint8 isr_dma_done_GetState(void)
{
    ...
    return ((*isr_dma_done_INTC_SET_EN & (uint32)isr_dma_done__INTC_MASK) != 0u) ? 1u:0u;
}
